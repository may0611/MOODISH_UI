...
const DesiredCookingTime = ({ navigation , route}) => {  
    ...
    const [inputText] = useState('생선 양파 돼지고기'); //임시
    ...
    const [selectedItem, setSelectedItem] = useState(false); // 단일 선택을 위한 상태
  ...
  const callApi_rec = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://172.25.86.64:5000/recommend', { //위치에 따라 ip변경 필요
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    ...
    const handleNext = () => {
        if (selectedItem) {
            navigation.navigate('RecipeRecommendation', { result:  apiResult }); // 'RecipeRecommendation' 화면으로 이동
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.buttonContainer}>
                <TouchableOpacity
                    style={[styles.button, (!selectedItem && styles.buttonDisabled)||(isLoading && styles.buttonDisabled)]}
